...
Our feature engineering resulted in a hybrid of binary and numerical features, with 70/77 features being binary/categorical (for example, the "room" indicator variables) and 7/77 being numerical (such as the "emotion intensity"). This meant that no model could accurately "capture" the underlying distribution in the features. Because of this, we chose to explore two different variants of the Naive Bayes models.

\subsubsection*{Variant 1: Gaussian Naive Bayes}
We wanted to first see which Naive Bayes could perform best if we kept our data normalized for consistency if put in an ensemble. Gaussian NB assumes that the features follow a \textbf{Gaussian distribution}, which the normalized continuous features approximately follow. Using 5-fold stratified cross-validation, we tuned the hyperparameter \textit{var\_smoothing} and achieved a maximum CV accuracy of \textbf{81.08\%} with $var\_smoothing = 0.325$. The normalization of the dataset reduced the variance of each feature, thus requiring a larger smoothing value to to artificially increase the variance.

\subsubsection*{Variant 2: Bernoulli Naive Bayes}
We determined that the Bernoulli NB would be a stronger candidate because it captures binary data features more accurately, and \textbf{70/77} of our dataset was binary. However this required the data to be kept as \textbf{unnormalized} which posed a larger issue in our ensembles. Using 5-fold stratified cross-validation, we achieved  \textbf{85.74\%} accuracy - significantly higher than the Gaussian NB. This was achieved with the smoothing hyperparameter $alpha=0.215$ and the threshold hyperparameter $binarize=0.0$ (which sets features greater than 0 equal to 1, and 0 otherwise). In our hyperparameter tuning, higher $binarize$ values would have incorrectly flipped the binary indicator flags from 1 to 0, effectively nullifying the information gain. The $alpha$ value of 0.215 was selected from a grid search as it was large enough to prevent a zero-probability scenario where a single unexpected feature nullifies the prediction of the entire class. The 0.215 was found to be the ideal balance between sensitivity and generalization.
...
\end{tabular}
\end{table}
As expected, both ensemble accuracies fell below the GBM accuracy because the Naive Bayes accuracies were significantly lower than Logistic Regression and GBM. The more surprising result was that the ensemble incorporating Bernoulli NB (\textbf{89.15\%}) performed slightly worse than the one incorporating Gaussian NB (\textbf{89.38\%}) despite
...
\textbf{Shengdi Zhu:}
\begin{enumerate}
\item Compiled an exploratory list of possible ML models and implemented initial training script based on cross validation error for models includings: Gradient Boosting, Logistic Regression, Linear Regression, Bayesian, Neural Networks (Transformer), Tree-based.
\item Wrote an exploratory data analysis script in R for data and feature visualization.
\item Contributed to final report write up.
\end{enumerate}

\textbf{Zheng Chuen Justin Long:}
\begin{enumerate}
\item Data cleaning and pre-processing, including parsing the raw Likert-scale responses, converting multi-valued categorical columns into binary indicators, and extracting keyword features from the free-text responses.
\end{enumerate}
